...
HbSteadyDateTime HbSteadyDateTime::fromDateTime(const QDateTime& datetime, quint64 steady) {
    ...
    if (!steady_time.mDateTime.isValid())
        qDebug() << "fromDateTime: Invalid datetime!";

    return steady_time;
}

HbSteadyDateTime HbSteadyDateTime::fromString(const QString& value, const QString& format) {
    HbSteadyDateTime steady_time;
    ...
    if (!steady_time.mDateTime.isValid())
        qDebug() << "Invalid datetime! Some fields missing?";
