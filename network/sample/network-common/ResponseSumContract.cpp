...
ResponseSumContract::ResponseSumContract() : HbNetworkContract(Protocol::CHANNEL_SUM, Protocol::CODE_SRV_RESPONSE_SUM) {
    setRouting(HbNetworkProtocol::ROUTING_UNICAST);
    mResult = 0;
}

ResponseSumContract::ResponseSumContract(const ResponseSumContract& source) : HbNetworkContract(source) {
    if (&source != this) {
...
QString ResponseSumContract::toString() const {
    return QString("%1,result=%2").arg(HbNetworkContract::toString()).arg(mResult);
}
