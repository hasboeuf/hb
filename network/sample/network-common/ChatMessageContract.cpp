...
ChatMessageContract::ChatMessageContract() :
        HbNetworkContract(Protocol::CHANNEL_CHAT, Protocol::CODE_CLT_CHAT_MESSAGE) {
    setRouting(HbNetworkProtocol::ROUTING_UNICAST);
    mMessage = "";
}

ChatMessageContract::ChatMessageContract(const ChatMessageContract& source) : HbNetworkContract(source) {
    if (&source != this) {
