...
RequestSumContract::RequestSumContract() : HbNetworkContract(Protocol::CHANNEL_SUM, Protocol::CODE_CLT_REQUEST_SUM) {
...
QString RequestSumContract::toString() const {
    return QString("%1,intA=%2,intB=%3").arg(HbNetworkContract::toString()).arg(mIntA).arg(mIntB);
}
