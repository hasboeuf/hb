...
ChatMessageBackContract::ChatMessageBackContract() :
        HbNetworkContract(Protocol::CHANNEL_CHAT, Protocol::CODE_SRV_CHAT_MESSAGE) {
    setRouting(HbNetworkProtocol::ROUTING_MULTICAST);
    mMessage = "";
    mAuthor = "";
}

ChatMessageBackContract::ChatMessageBackContract(const ChatMessageBackContract& source) : HbNetworkContract(source) {
    if (&source != this) {
...
QString ChatMessageBackContract::toString() const {
    return QString("%1,message=%2,author=%3").arg(HbNetworkContract::toString()).arg(mMessage).arg(mAuthor);
}
