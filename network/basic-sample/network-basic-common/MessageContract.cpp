...
MessageContract::MessageContract() : HbNetworkContract(Protocol::SERVICE_MESSAGE, Protocol::CODE_SRV_MESSAGE) {
    setRouting(HbNetworkProtocol::ROUTING_MULTICAST);
    mMessage = "";
}

MessageContract::MessageContract(const MessageContract& source) : HbNetworkContract(source) {
    if (&source != this) {
...
QString MessageContract::toString() const {
    return QString("%1,message=%2").arg(HbNetworkContract::toString()).arg(mMessage);
}
