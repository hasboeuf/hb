...
ResponseContract::ResponseContract() : HbNetworkContract(Protocol::SERVICE_QUESTION, Protocol::CODE_SRV_RESPONSE) {
    setRouting(HbNetworkProtocol::ROUTING_UNICAST);
    mResponse = "";
}

ResponseContract::ResponseContract(const ResponseContract& source) : HbNetworkContract(source) {
    if (&source != this) {
...
QString ResponseContract::toString() const {
    return QString("%1,response=%2").arg(HbNetworkContract::toString()).arg(mResponse);
}
