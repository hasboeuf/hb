...
HbKickContract::HbKickContract() :
        HbNetworkContract(HbNetworkProtocol::SERVICE_KICK, HbNetworkProtocol::CODE_SRV_KICK) {
    setRouting(HbNetworkProtocol::ROUTING_UNICAST);
    mReason = HbNetworkProtocol::KICK_UNDEFINED;
}

HbKickContract::HbKickContract(const HbKickContract& source) : HbNetworkContract(source) {
    if (&source != this) {
