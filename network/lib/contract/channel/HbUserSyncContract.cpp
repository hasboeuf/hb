...
HbUserSyncContract::HbUserSyncContract() :
        HbNetworkContract(HbNetworkProtocol::SERVICE_CHANNEL, HbNetworkProtocol::CODE_SRV_USER_CONNECTED) {
    setRouting(HbNetworkProtocol::ROUTING_MULTICAST);
}

HbUserSyncContract::HbUserSyncContract(const HbUserSyncContract& source) : HbNetworkContract(source) {
    if (&source != this) {
