...
HbClient::HbClient(const HbGeneralClientConfig& config) : HbPeer(config), mConnectionPool(config) {
    if (isReady()) {
        connect(&mConnectionPool,
                &HbClientConnectionPool::statusChanged,
                this,
                &HbClient::clientStatusChanged); // Signal to signal.
        connect(&mConnectionPool,
                &HbClientConnectionPool::meStatusChanged,
                this,
                &HbClient::meStatusChanged); // Signal to signal.
    }
}
...
bool HbClient::OAuthRequested(HbAuthService::AuthType type) {
    if (!isReady())
        return false;
    if (type == HbAuthService::AUTH_NONE && type >= HbAuthService::AUTH_USER) {
        return false;
    }
