...
const HbLogMessage* HbLogMessage::fromRaw(const QString& raw) {
    ...
    if (fields.size() == 7) {
        ...
        HbSteadyDateTime datetime =
            HbSteadyDateTime::fromString(time_str, QStringLiteral("yyyy-MM-dd-HH:mm:ss:zzz.uuuuuu"));
...
void HbLogMessage::setPattern(const QString& pattern) {
    ...
    if (pattern.contains(TOKEN_LEVEL))
        outputs |= OUTPUT_LEVEL;
    if (pattern.contains(TOKEN_TIME))
        outputs |= OUTPUT_TIME;
    if (pattern.contains(TOKEN_APP))
        outputs |= OUTPUT_APP;
    if (pattern.contains(TOKEN_FILE))
        outputs |= OUTPUT_FILE;
    if (pattern.contains(TOKEN_FUNCTION))
        outputs |= OUTPUT_FUNCTION;
    if (pattern.contains(TOKEN_LINE))
        outputs |= OUTPUT_LINE;
    if (pattern.contains(TOKEN_MESSAGE))
