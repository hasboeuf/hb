...
LogViewerTab::LogViewerTab(qint32 id,
    ...
    connect(
        qcb_level, (void (QComboBox::*)(int)) & QComboBox::currentIndexChanged, this, &LogViewerTab::onLevelChanged);
    ...
    connect(qtv_log, &QTableView::customContextMenuRequested, this, &LogViewerTab::onCustomContextMenuRequested);
}
...
bool LogViewerTab::isValidEntry(QList<QStandardItem*>& row) {
    if (!row.at(COLUMN_LEVEL) || !row.at(COLUMN_TIME) || !row.at(COLUMN_OWNER) || !row.at(COLUMN_LINE)
        || !row.at(COLUMN_WHERE) || !row.at(COLUMN_TEXT) || !row.at(COLUMN_FILE) || !row.at(COLUMN_FUNC)) {
...
void LogViewerTab::onRowDoubleClicked(const QModelIndex& index) {
    ...
    if (!QProcess::startDetached(cmd)) {
        QMessageBox::critical(
            this, QStringLiteral("Error"), QStringLiteral("Error while opening file."), QMessageBox::Ok);
    }
#endif
}
...
QSet<QString> LogViewerTab::findAbsoluteFilePath(const QString& file_path, const QString& root_path) {
    ...
    for (const QString subdir : subdirs) {
        QString subdir_path = QStringLiteral("%1%2/").arg(root_path_info.absoluteFilePath()).arg(subdir);

        file_path_found.unite(findAbsoluteFilePath(file_path, subdir_path));
    }

    // Check current dir
    QString full_file_path = QStringLiteral("%1/%2").arg(root_path).arg(file_path);
